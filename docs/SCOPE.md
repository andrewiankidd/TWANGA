...
- **Not a tab library.** Tabs are a legal grey zone. The app ships empty. Users bring their own files. Community sharing happens off-platform, like emulator ROMs.
- **No proprietary tab formats.** Guitar Pro's `.gp5` / `.gpx` are reverse-engineerable but commercially protected, same legal posture as the ASIO SDK. Native format is [alphaTex](https://github.com/CoderLine/alphaTab) (open text, alphaTab project). MusicXML is the planned open-standard interop target for sheet-music editors. Don't add proprietary parsers even if a community PR offers one.
