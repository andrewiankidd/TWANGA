...
- **Quick mode vs precise mode labelling.** Today TWANGA exposes the calibrated latency value plus the chosen policy (tight ±50 ms / casual ±150 ms) — but doesn't bake "this hardware class fits this tolerance" into the UI. A future labelling pass could surface "🎤 built-in mic — casual mode recommended" vs "🎸 audio interface — tight mode within reach" automatically based on the measured latency + jitter from `twanga calibrate`, rather than making the user pick. Honest > pretending the experience is identical across hardware.
...
- **Hammer-on / pull-off / muted-note onset detection.** Today's energy-derivative onset detector ([`twanga-dsp::onset`](../crates/twanga-dsp/src/onset.rs)) only fires on fresh amplitude transients, so a hammer-on (left-hand-only re-articulation, no fresh pluck) produces no onset and wait-mode / proximity-score skip the column. Documented v1 limitation; the v1 user guidance is "pluck every note." Real fix needs a second detection path that triggers on a spectral-content change without an amplitude spike — likely spectral-flux on a short FFT window. Defer until the Articulation work above lands, because that's where the data model already needs to know "this note is a hammer-on, expect no fresh attack."
- **Sustained-tone fallback for open-string drones.** Wait-mode's onset gating ([`from_onset_window` check](../crates/twanga-cli/src/main.rs)) treats a long-decay open string (banjo 5th, ukulele reentrant G) as "already played" once the initial pluck fires — if the tab calls for the same open string two columns in a row and the user lets it ring, the second column never advances. Same behaviour as pre-Ship-1 wait-mode for drones; not a regression, but a known sharp edge. Configurable timeout-based fallback ("if no new onset within N ms and the pitch still matches, accept as a re-strike") would close the gap; deferred because it complicates the matching contract that test coverage just nailed down.
...
- **Continuous auto-tuning + std-dev sanity check + calibration redesign.** Today's auto-tuning warmup (in playback's live scorer) observes the user's first 4 non-rest columns, takes the median offset, and locks that as the scoring shift for the rest of the session. Real follow-ups:
  - **Continuous re-tuning** — a rolling-window mean over the last N columns, so a user who naturally settles into a slightly different pocket mid-song gets re-centred. Stable vs adaptive trade-off; needs a damping factor so a single misplayed bar doesn't yank the window.
  - **Std-dev sanity check** — only apply auto-offset when the observed offsets cluster tightly; if jitter is high (user playing chaotically during warmup), keep `autoOffsetMs = 0` and surface "Auto-tune unavailable (high jitter)" instead. Avoids silently centring on garbage.
  - **Drop pluck-along's reaction-time-inclusive model** — currently `twanga calibrate` with pluck-along bakes reaction time into `extra_latency_ms`, which over-corrects for anticipating players and forces them to recalibrate. Auto-tuning now absorbs that role; pluck-along should be repositioned as a "rough starting estimate" or replaced with a hardware-only measurement (tap-the-mic with floor-not-median to exclude reaction time). The wizard's `extra_latency_ms` should default to a small system-delay-only value with auto-tuning expected to refine it per-session.
  - **Per-column offset trend display** — surface "you're consistently rushing by ~80 ms" alongside the live badge so users can self-correct rather than relying on opaque auto-tuning.
- **Calibration-aware onset thresholds.** The onset detector ([`twanga-dsp::onset`](../crates/twanga-dsp/src/onset.rs)) currently uses fixed defaults: `min_delta_rms = 0.005`, `ratio_threshold = 2.5`. They work well across the 33 `--from-file` integration scenarios, but per the original onset-detection design these should derive from the same per-session noise-floor measurement that already drives `Tuner::silence_rms` (`twanga-dsp::Tuner::start_noise_calibration`). The win: catch quieter plucks on clean DIs (where the absolute floor is much lower than the default) and reject more false positives on noisy mics (where the default ratio gets tripped by ambient peaks). Implementation hook: extend the noise-floor calibration to emit a recommendation tuple (`silence_rms`, `min_delta_rms`, `ratio_threshold`) and have `Tuner` apply all three when calibration finishes.
